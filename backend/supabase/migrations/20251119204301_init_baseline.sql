...
CREATE UNIQUE INDEX booking_deliverables_pkey ON public.booking_deliverables USING btree (id);

CREATE INDEX idx_booking_deliverables_booking_id ON public.booking_deliverables USING btree (booking_id);

alter table "public"."booking_deliverables" add constraint "booking_deliverables_pkey" PRIMARY KEY using index "booking_deliverables_pkey";

alter table "public"."booking_deliverables" add constraint "booking_deliverables_booking_id_fkey" FOREIGN KEY (booking_id) REFERENCES bookings(id) ON DELETE CASCADE not valid;

alter table "public"."booking_deliverables" validate constraint "booking_deliverables_booking_id_fkey";

alter table "public"."bookings" add constraint "bookings_client_status_check" CHECK ((client_status = ANY (ARRAY['placed'::text, 'confirmed'::text, 'payment_required'::text, 'in_progress'::text, 'completed'::text, 'cancelled'::text, 'locked'::text, 'download'::text]))) not valid;
...
create policy "Clients can read deliverables for their bookings"
...
using ((EXISTS ( SELECT 1
   FROM bookings
  WHERE ((bookings.id = booking_deliverables.booking_id) AND (bookings.client_user_id = auth.uid())))));


create policy "Creatives can manage deliverables for their bookings"
on "public"."booking_deliverables"
as permissive
for all
to public
using ((EXISTS ( SELECT 1
   FROM bookings
  WHERE ((bookings.id = booking_deliverables.booking_id) AND (bookings.creative_user_id = auth.uid())))));
