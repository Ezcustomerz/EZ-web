...
alter table "public"."creative_client_relationships" alter column "status" set default 'inactive'::text;

CREATE INDEX idx_service_photos_display_order ON public.service_photos USING btree (service_id, display_order);

CREATE INDEX idx_service_photos_is_primary ON public.service_photos USING btree (service_id, is_primary);

CREATE INDEX idx_service_photos_service_id ON public.service_photos USING btree (service_id);

CREATE UNIQUE INDEX idx_service_photos_unique_primary ON public.service_photos USING btree (service_id) WHERE (is_primary = true);

CREATE UNIQUE INDEX service_photos_pkey ON public.service_photos USING btree (id);

alter table "public"."service_photos" add constraint "service_photos_pkey" PRIMARY KEY using index "service_photos_pkey";

alter table "public"."service_photos" add constraint "service_photos_display_order_check" CHECK ((display_order >= 0)) not valid;

alter table "public"."service_photos" validate constraint "service_photos_display_order_check";

alter table "public"."service_photos" add constraint "service_photos_photo_size_check" CHECK ((photo_size_bytes > 0)) not valid;

alter table "public"."service_photos" validate constraint "service_photos_photo_size_check";

alter table "public"."service_photos" add constraint "service_photos_service_id_fkey" FOREIGN KEY (service_id) REFERENCES creative_services(id) ON DELETE CASCADE not valid;

alter table "public"."service_photos" validate constraint "service_photos_service_id_fkey";
...
CREATE OR REPLACE FUNCTION public.get_client_connected_services(client_user_id_param text)
 RETURNS TABLE(id uuid, title text, description text, price numeric, delivery_time text, status text, color text, is_active boolean, created_at timestamp with time zone, updated_at timestamp with time zone, creative_user_id text, creative_name text, photos jsonb)
 LANGUAGE plpgsql
AS $function$
BEGIN
    RETURN QUERY
...
CREATE OR REPLACE FUNCTION public.get_client_connected_services_with_photos(client_user_id uuid)
 RETURNS TABLE(id uuid, title text, description text, price numeric, delivery_time text, status text, color text, is_active boolean, created_at timestamp with time zone, updated_at timestamp with time zone, creative_user_id uuid, creative_name text, photos jsonb)
 LANGUAGE plpgsql
AS $function$
BEGIN
  RETURN QUERY
...
create policy "Creatives can delete service photos"
...
using ((EXISTS ( SELECT 1
   FROM creative_services cs
  WHERE ((cs.id = service_photos.service_id) AND (cs.creative_user_id = auth.uid())))));


create policy "Creatives can insert service photos"
on "public"."service_photos"
as permissive
for insert
to public
...
create policy "Creatives can update service photos"
on "public"."service_photos"
as permissive
for update
to public
...
create policy "Users can view service photos"
on "public"."service_photos"
as permissive
for select
to public
using ((EXISTS ( SELECT 1
   FROM creative_services cs
  WHERE ((cs.id = service_photos.service_id) AND ((cs.creative_user_id = auth.uid()) OR (cs.status = 'Public'::text))))));


CREATE TRIGGER service_photos_limit_trigger BEFORE INSERT ON public.service_photos FOR EACH ROW EXECUTE FUNCTION check_service_photo_limit();

CREATE TRIGGER update_service_photos_updated_at BEFORE UPDATE ON public.service_photos FOR EACH ROW EXECUTE FUNCTION update_service_photos_updated_at();
